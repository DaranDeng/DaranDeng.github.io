...
| **GCD (Euclid)**| Recursive | **$O(\log n)$** | **最坏情况**是输入为连续的斐波那契数。递归深度增长缓慢（对数级）。Fib(k) ≈ 1.618 × Fib(k-1)。b × (0.618)^t<=1。当这个值小于 1 时，递归结束。所以 (0.618)^t ≈ 1/b → 取对数：t ≈ log_{1/0.618} b。也就是说，递归深度和 log b 成正比，栈帧用了log b个，而递归调用次数用来表示时间复杂度，由于每个函数只调用了一次自己，所以这里栈帧=递归调用次数，时间复杂度也是O(log b)。而 b 是较小的那个数，所以空间复杂度是 O(log b)，而不是 O(b)。**最好情况**是b正好可以被a整除,O(1)|
